...
assert.match(appSource, /const COURSE_ELEVATION_DISPLAY_SCALE = 5/, "elevation display scale should be fixed at 5x");
assert.doesNotMatch(appSource, /setElevationScale/, "fixed elevation display scale should not expose a setter");
...
assert.match(sceneSource, /const START_FINISH_LABEL = "START \/ FINISH"/, "nearby start and finish markers should share one label");
assert.match(sceneSource, /areReferencePointsNear\(startPoint, finishPoint, elevationScale, baselineDisplayY\)/, "reference start and finish marker merge should use render-space proximity");
assert.match(sceneSource, /areProjectedLapPointsNear\(actualStartPoint, actualFinishPoint, elevationScale, baselineDisplayY\)/, "actual start and finish marker merge should use render-space proximity");
assert.match(sceneSource, /label=\{START_FINISH_LABEL\}/, "merged start/finish marker should render as a single combined label");
assert.match(sceneSource, /\{ label: "START", point: mergeStartFinish \? undefined : startPoint \}/, "merged start/finish should suppress the start elevation-guide item");
assert.match(sceneSource, /\{ label: "FINISH", point: mergeStartFinish \? undefined : finishPoint \}/, "merged start/finish should suppress the finish elevation-guide item");
assert.doesNotMatch(sceneSource, /hideRelativeHeight/, "merged start/finish should not leave a smaller background elevation label behind");
