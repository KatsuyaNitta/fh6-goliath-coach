...
生成される artifact は `viewer/public/reference/goliath_course_geometry.json` で、schema は `goliath-course-geometry-v1` です。1 m 基準パスと同じ点数の compact tuple を持ち、主要フィールドは `reference_index`、`course_distance_m`、`section_id`、`tangent_x/tangent_z`、`heading_deg`、`gradient_ratio/gradient_pct/gradient_angle_deg`、`signed_curvature_1pm`、`estimated_radius_m`、`turn_direction`、`slope_direction`、`quality_flags` です。Viewer はこの artifact が欠落または不整合の場合でも起動し、セクション色表示、速度/操作のコース色、テレメトリチャートを維持します。勾配と曲率は詳細読出し、解析データ、将来のコーナー検出に残します。

Viewer のコース色は `Section`、`速度`、`操作` を切り替えられます。`Section` は S1-S6 の既存セクション色をそのまま使い、ラップ未読込でも利用できます。`速度` は読込済み実ラップの `speed_kmh` を固定スケールで色付けします。停止点は `0 #1d4ed8`、`80 #06b6d4`、`160 #22c55e`、`240 #facc15`、`300 #f97316`、`360+ #ef4444` で、赤は良し悪しではなく速度の大きさを示します。`操作` はアクセル/ブレーキから、同時入力、ブレーキ、アクセルオフ、部分アクセル、全開、不明へ分類します。同時入力を最優先し、ブレーキは `> 2%`、アクセルオフはアクセル `<= 2%` かつブレーキ `<= 2%`、部分アクセルは `> 2%` かつ `< 95%`、全開は `>= 95%` です。速度/操作のコース色はデータ色として描画し、カメラ照明や角度で意味が変わらないようにします。全開は長く続きやすいため意図的に控えめな色にし、欠損値は 0 ではなく不明として扱います。勾配と曲率は全体色モードから外しますが、artifact 内の解析値、左右/上り下り分類、品質フラグ、詳細読出し、将来のルールベース解析には残します。
...
**重点練習箇所** is a mechanical rewind-derived candidate extractor, not a final driving-quality diagnosis. The viewer keeps all qualifying rewind clusters and shows the evidence so the user can make the final judgment.

A rewind cluster is included when it has at least two rewind events at the same existing cluster, even if the classification is uncertain or external-impact suspected. A single-event cluster is included only when it has at least one `driving_error_suspected` event with `medium` or `high` confidence. Candidates are sorted deterministically by rewind event count, driving-error count, confidence, rewound time, course distance, and cluster id. No composite score is calculated, and the existing rewind classification remains probabilistic.
