...
Section Focus は現在、選択区間の強調表示を維持する表示モードです。右側の S1-S6 セクションをクリックした場合と、テレメトリチャート上の点をクリックしてピン留めした場合に Section Focus へ切り替わります。チャートの hover や Full lap / Selected section の範囲切り替えは map display mode を変更しません。選択区間へカメラをフィットする機能はまだ実装していません。
...
Current capabilities include a 1 m sampled reference path, S1-S6 sections and P1-P5 boundary markers, continuity-constrained telemetry projection, completed-lap extraction, actual-path overlay, section timing, selected-section emphasis, Overview map mode, and vehicle/tune metadata save/load.

The viewer opens in **Overview** mode. Overview frames the full Goliath course, renders S1-S6 with equal emphasis, and slowly auto-rotates in 3D unless `prefers-reduced-motion: reduce` is active. Manual camera interaction stops that rotation. Clicking an explicit S1-S6 section control or pinning a telemetry chart point enters **Section Focus** and keeps the current camera pose. Chart hover and chart range controls do not change the map display mode. Section Focus keeps the existing selected-section emphasis and does not yet fit the camera to the selected section.
